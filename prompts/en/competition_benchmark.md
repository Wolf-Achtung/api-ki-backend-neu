Developer:
<!-- competition_benchmark.md – v5.0 GOLD STANDARD+ (branch-aware, size-aware, score-aware)
     Respond exclusively with valid HTML.
     NO <html>, <head> or <body>; NO comments in output.

     PURPOSE:
       - Compare the company with industry average and top 10%.
       - Derive position per category exclusively from the scores.
       - Identify the two largest gaps & two strongest strengths.
       - Create a size-aware catch-up strategy (Q2–Q4).
       - Use CONTEXT_BLOCK (tools, workflows, pain points) for industry focus.

     MANDATORY VARIABLES:
       {{BRANCHE_LABEL}}
       {{report_date}}
       {{score_gesamt}}
       {{score_befaehigung}}
       {{score_governance}}
       {{score_sicherheit}}
       {{score_nutzen}}
       {{UNTERNEHMENSGROESSE_LABEL}}

     BENCHMARK NUMBERS (DO NOT CHANGE!):
       Overall: Ø 65, Top 10% = 82
       Enablement: Ø 68, Top 10% = 85
       Governance: Ø 58, Top 10% = 78
       Security: Ø 62, Top 10% = 80
       Value Creation: Ø 70, Top 10% = 88

     SCORE LOGIC (binding):
       Positioning (per category):
         - "> Top 10%" → "significantly above industry level"
         - "between Ø and Top 10%" → "above industry level"
         - "< Ø" → "below industry average"

       Gaps:
         - Choose the two categories with largest distance to Ø, only if Score < Ø.
       Strengths:
         - Choose the two categories with largest distance upward (Score > Ø).

     BRANCH-AWARE:
       - Use CONTEXT_BLOCK: industry-typical processes, pain points, data types.
       - Industry modifiers:
         FINANCE/HEALTH/PUBLIC SECTOR = Focus on Governance & Security.
         MARKETING/CREATIVE = Focus on Value Creation & Enablement.
         MANUFACTURING/PRODUCTION = Focus on Data Quality & Process Integration.
         E-COMMERCE/RETAIL = Focus on Consistency, Data Feeds, Automation.

     SIZE-AWARE STRATEGY:
       SOLO:
         - Focus on personal routines, pragmatic standards, direct benefit effects.
       TEAM (2–10):
         - Focus on roles, coordination, shared workflows & reviews.
       SME (11–100):
         - Focus on cross-functional coordination, governance, scalable processes.

     OUTPUT:
       A single HTML block with <section>…</section>.
-->

<section class="section competition-benchmark">
  <h2>Competition &amp; Benchmarking</h2>

  <p>
    <strong>Data basis:</strong> Benchmark from 30 assessments in
    <strong>{{BRANCHE_LABEL}}</strong>, as of <strong>{{report_date}}</strong>.
  </p>

  <h3>Score Comparison (Company vs. Industry)</h3>
  <table class="table">
    <thead>
      <tr>
        <th>Category</th>
        <th>Your Score</th>
        <th>Ø Industry</th>
        <th>Top&nbsp;10%</th>
        <th>Position</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>Overall</td>
        <td>{{score_gesamt}}</td>
        <td>65</td>
        <td>82</td>
        <td>
          <!-- automatically derivable -->
          {% if score_gesamt > 82 %}significantly above industry level
          {% elif score_gesamt > 65 %}above industry level
          {% else %}below industry average{% endif %}
        </td>
      </tr>

      <tr>
        <td>Enablement</td>
        <td>{{score_befaehigung}}</td>
        <td>68</td>
        <td>85</td>
        <td>
          {% if score_befaehigung > 85 %}significantly above industry level
          {% elif score_befaehigung > 68 %}above industry level
          {% else %}below industry average{% endif %}
        </td>
      </tr>

      <tr>
        <td>Governance</td>
        <td>{{score_governance}}</td>
        <td>58</td>
        <td>78</td>
        <td>
          {% if score_governance > 78 %}significantly above industry level
          {% elif score_governance > 58 %}above industry level
          {% else %}below industry average{% endif %}
        </td>
      </tr>

      <tr>
        <td>Security</td>
        <td>{{score_sicherheit}}</td>
        <td>62</td>
        <td>80</td>
        <td>
          {% if score_sicherheit > 80 %}significantly above industry level
          {% elif score_sicherheit > 62 %}above industry level
          {% else %}below industry average{% endif %}
        </td>
      </tr>

      <tr>
        <td>Value Creation</td>
        <td>{{score_nutzen}}</td>
        <td>70</td>
        <td>88</td>
        <td>
          {% if score_nutzen > 88 %}significantly above industry level
          {% elif score_nutzen > 70 %}above industry level
          {% else %}below industry average{% endif %}
        </td>
      </tr>
    </tbody>
  </table>

  <h3>Your Biggest Gaps</h3>
  <p>
    The following areas show the largest distance to the industry average and offer
    correspondingly high improvement potential:
  </p>
  <ul>
    {% if score_befaehigung < 68 %}
      <li><strong>Enablement:</strong> significant gap compared to industry Ø ({{score_befaehigung}} vs. 68).</li>
    {% endif %}
    {% if score_governance < 58 %}
      <li><strong>Governance:</strong> significant gap compared to industry Ø ({{score_governance}} vs. 58).</li>
    {% endif %}
    {% if score_sicherheit < 62 %}
      <li><strong>Security:</strong> significant gap compared to industry Ø ({{score_sicherheit}} vs. 62).</li>
    {% endif %}
    {% if score_nutzen < 70 %}
      <li><strong>Value Creation:</strong> significant gap compared to industry Ø ({{score_nutzen}} vs. 70).</li>
    {% endif %}
  </ul>

  <h3>Your Strongest Strengths</h3>
  <p>
    These areas are noticeably above the industry average and can serve as a
    foundation for further development:
  </p>
  <ul>
    {% if score_befaehigung > 68 %}
      <li><strong>Enablement:</strong> noticeable lead over industry average ({{score_befaehigung}} vs. 68).</li>
    {% endif %}
    {% if score_governance > 58 %}
      <li><strong>Governance:</strong> noticeable lead over industry average ({{score_governance}} vs. 58).</li>
    {% endif %}
    {% if score_sicherheit > 62 %}
      <li><strong>Security:</strong> noticeable lead over industry average ({{score_sicherheit}} vs. 62).</li>
    {% endif %}
    {% if score_nutzen > 70 %}
      <li><strong>Value Creation:</strong> noticeable lead over industry average ({{score_nutzen}} vs. 70).</li>
    {% endif %}
  </ul>

  <h3>Catch-Up Strategy (Next 12 Months – size-aware)</h3>
  <ol>
    <li>
      <strong>Q2:</strong>
      {% if UNTERNEHMENSGROESSE_LABEL.startswith('1') %}
        Focus on personal standardization: document 2–3 core processes, use simple AI checklists.
      {% elif UNTERNEHMENSGROESSE_LABEL.startswith('2') %}
        Clarify roles (AI owner, reviewer), uniform templates and short review loops.
      {% else %}
        Define cross-functional pilot area (e.g., marketing, production, back office); anchor first governance standards.
      {% endif %}
    </li>

    <li>
      <strong>Q3:</strong>
      {% if UNTERNEHMENSGROESSE_LABEL.startswith('1') %}
        Workflow optimization: solidify AI-powered routines, integrate most important industry tools.
      {% elif UNTERNEHMENSGROESSE_LABEL.startswith('2') %}
        Shared documentation + regular team reviews; reduce tool duplication.
      {% else %}
        Harmonize area-specific processes, clear data interfaces, uniform approvals.
      {% endif %}
    </li>

    <li>
      <strong>Q4:</strong>
      {% if UNTERNEHMENSGROESSE_LABEL.startswith('1') %}
        Routine consolidation: recurring usage + annual planning.
      {% elif UNTERNEHMENSGROESSE_LABEL.startswith('2') %}
        Scaling in team: automated quality control + uniform AI communication.
      {% else %}
        Scaling program: expand governance, audit routines, cross-functional standards.
      {% endif %}
    </li>
  </ol>

  <p>
    <strong>Strategic target corridor:</strong>
    {% if score_gesamt >= 80 %}
      Towards top 5% of the industry.
    {% elif score_gesamt >= 60 %}
      Towards top 10% of the industry.
    {% else %}
      Towards top 25% – focus on stabilization and structured development.
    {% endif %}
  </p>
</section>
