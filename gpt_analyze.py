...
NSFW_KEYWORDS = {
    "porn", "xxx", "sex", "nude", "naked", "adult", "nsfw", "erotic", "escort", "dating",
    "porno", "nackt", "fick", "titten", "onlyfans", "torrent", "crack"
}
NSFW_DOMAINS = {
    "xvideos.com", "pornhub.com", "xnxx.com", "redtube.com", "youporn.com", "onlyfans.com"
}
...
def _map_german_to_english_keys(answers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    m["goals"] = (
        ", ".join(answers.get("ki_ziele", [])) if answers.get("ki_ziele") else answers.get("strategische_ziele", "")
    )
...
def _calculate_realistic_score(answers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if budget in ["10k-50k", "50k-100k", "over_100k"]:
        gov += 6
        details["governance"].append("✅ Ausreichendes Budget")
    elif budget == "under_10k":
        gov += 3
        details["governance"].append("⚠️ Niedriges Budget")
    else:
...
def _generate_content_sections(briefing: Dict[str, Any], scores: Dict[str, Any]) -> Dict[str, str]:
    ...
    if total_h > 0:
        ...
        lo = max(1, int(round(total_h * 0.7)))
        hi = int(round(total_h * 1.2))
...
def _send_emails(db: Session, rep: Report, br: Briefing,
    ...
    if best_pdf:
        attachments_admin.append(
            {"filename": f"KI-Status-Report-{getattr(rep, 'id', None)}.pdf", "content": best_pdf, "mimetype": "application/pdf"}
        )
    try:
    ...
    try:
        ...
        if user_email:
            ok, err = send_mail(
                ...
                text=None,
                attachments=([] if pdf_url else attachments_admin[:1]),
            )
            if ok:
                log.info("[%s] 📧 Mail sent to user %s", run_id, _mask_email(user_email))
            else:
    ...
    try:
        if os.getenv("ENABLE_ADMIN_NOTIFY", "1") in ("1", "true", "TRUE", "yes", "YES"):
            for addr in _admin_recipients():
                ...
                if ok:
                    log.info("[%s] 📧 Admin notify sent to %s", run_id, _mask_email(addr))
                else:
...
def run_async(briefing_id: int, email: Optional[str] = None) -> None:
    ...
    try:
        ...
        if hasattr(rep, "user_email"):
            rep.user_email = (email or "")  # store masked only in mail
