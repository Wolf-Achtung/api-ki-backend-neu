...
"""
gpt_analyze.py – v4.13.0-gs (Gold‑Standard+)
---------------------------------------------------------------------
Änderungen (gegenüber v4.12.0):
- **Blautons‑Akzente**: Theme‑Variablen per Branch (`THEME_CSS_VARS` → Template).
- **Footer‑Logos**: `FOOTER_LEFT_LOGO_SRC`, `FOOTER_MID_LOGO_SRC`, `FOOTER_RIGHT_LOGO_SRC`.
- **Defaults** für optionale Template‑Platzhalter (KPI/Feedback/Glossar/etc.).
- **Benchmarks**: Branche (aus JSON) + **Unternehmensgröße (Schätzung)** bleiben erhalten.
- **Quellenkasten**: Aggregation & Dedupe aus verlinkten Abschnitten.
- **Mail‑Versand**: Nutzer + Admins (mit Fallback‑Attachment, Masking).
- **Stabiles HTML**: Sanitize/Repair + Quick‑Wins‑Summierung (h/Monat).
- **Keine Fantasienamen**: Aufgaben rollenbasiert (👤 Rolle statt Name).
---------------------------------------------------------------------
...
def _calculate_realistic_score(answers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    log.info(\"📊 REALISTIC SCORES v4.13.0: Gov=%s Sec=%s Val=%s Ena=%s Overall=%s\",
...
def _build_benchmark_html(answers: Dict[str, Any]) -> str:
    ...
    if branche and isinstance(data, dict) and branche in data:
        ...
        row_html.append(f\"<tr><td><strong>Branche</strong>: {branche or '—'}</td><td>—</td><td>—</td></tr>\")
    if size_label:
...
def _theme_vars_for_branch(branch_label: str) -> str:
    \"\"\"Nur Blautöne variieren: liefert <style> mit :root‑Overrides.\"\"\"
    b = (branch_label or \"\").lower()
    brand, weak, accent = \"#2563eb\", \"#dbeafe\", \"#1e3a5f\"     # default
    if \"it\" in b or \"software\" in b:
        brand, weak, accent = \"#1d4ed8\", \"#c7d2fe\", \"#16327a\"  # tiefes Blau
    elif \"marketing\" in b or \"werbung\" in b:
        brand, weak, accent = \"#0ea5e9\", \"#bae6fd\", \"#0c4a6e\"  # sky‑blau
    elif \"industrie\" in b or \"produktion\" in b:
        brand, weak, accent = \"#1e40af\", \"#c7d2fe\", \"#112a63\"  # dunkler
...
def _generate_content_sections(briefing: Dict[str, Any], scores: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Next Actions (rollenbasiert; keine Namen)
    nxt = _call_openai(
...
def analyze_briefing(db: Session, briefing_id: int, run_id: str) -> tuple[int, str, Dict[str, Any]]:
    ...
    # Scores
    log.info(\"[%s] Calculating realistic scores (v4.13.0)...\", run_id)
    ...
    # Defaults for optional template placeholders
    sections.setdefault('KPI_HTML', \"\")  # falls kein KPI‑Builder verwendet wird
    ...
    # Optional altes HTML‑Snippet bleibt kompatibel
    sections['FOOTER_BRANDS_HTML'] = os.getenv(\"FOOTER_BRANDS_HTML\", \"\")
    # Owner/Contact für Fußzeile
    ...
    log.info(\"[%s] ✅ Analysis created (v4.13.0-gs): id=%s\", run_id, an.id)
...
def run_async(briefing_id: int, email: Optional[str] = None) -> None:
    ...
    try:
        log.info(\"[%s] 🚀 Starting analysis v4.13.0-gs for briefing_id=%s\", run_id, briefing_id)
        ...
        if hasattr(rep, \"user_email\"): rep.user_email = (email or \"\")  # store masked only in mail
