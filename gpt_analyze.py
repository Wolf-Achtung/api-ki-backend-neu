...
def _build_prompt_vars(briefing: Dict[str, Any], scores: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # X1: DEFINITIVE FIX — Truncate hauptleistung at SOURCE before ANY GPT call
    # Previous attempts (U1, V1, W1, W2) all failed because:
    ...
    if USE_PROMPT_SYSTEM and prompt_key and _prompt_enhancer:
        try:
            ...
            platin_min_words = {
                ...
                "technologie_prozesse": 200,       # RUN-622: Was missing → 27 words passed as OK
                "ki_aktivitaeten_ziele": 150,       # RUN-622: Safety net
                "wettbewerb_benchmark": 200,        # RUN-622: Safety net
                "transparency_box": 80,             # RUN-622: Safety net
                "monetarisierung": 150,             # RUN-622: Safety net
...
"""
    ...
    try:
        ...
        try:
            import re as _re732
            from difflib import SequenceMatcher as _SM732
            _gc_dec = sections.get("GAMECHANGER_DECISION_HTML", "") or ""
            _ks_sum = sections.get("KI_STACK_SUMMARY_HTML", "") or ""
            if _gc_dec and _ks_sum and len(_gc_dec) > 100 and len(_ks_sum) > 100:
                _gc_txt = _re732.sub(r'<[^>]+>', '', _gc_dec).strip()[:500]
                _ks_txt = _re732.sub(r'<[^>]+>', '', _ks_sum).strip()[:500]
                _sim = _SM732(None, _gc_txt, _ks_txt).ratio()
                if _sim > 0.65:
                    _hl = sections.get("HAUPTLEISTUNG", "") or sections.get("hauptleistung", "") or "Ihr Unternehmen"
                    _ks_new = (
                    ...
                    sections["ki_stack_summary"] = _ks_new
                    log.info(f"[FIX-B732-GC-DEDUP] Replaced KI_STACK_SUMMARY (similarity={_sim:.0%}) with compact reference")
                else:
                    log.info(f"[FIX-B732-GC-DEDUP] Similarity={_sim:.0%} - no dedup needed")
            else:
                log.info("[FIX-B732-GC-DEDUP] Skipped - sections empty or too short")
        except Exception as _b732e:
            log.warning(f"[FIX-B732-GC-DEDUP] Failed: {_b732e}")
    ...
    _gc_t = sections.get("GAMECHANGER_HTML", ""); _gc_tw = len(re.sub(r"<[^>]+>", "", _gc_t).split()) if _gc_t else 0; log.info("[%s] [FIX-642-TRACE] GC after QUALITY-ENFORCER-2: %d words", "GCS", _gc_tw)
    return sections
...
# ============================================================
    ...
    try:
        ...
        if isinstance(_bench, dict):
            ...
            _real = [w for w in _weaknesses if isinstance(w, str) and w.strip().lower() not in _placeholder]
            if not _real:
                _bench.setdefault('weaknesses', []).append(
                    'Automatisierungsgrad unter Branchendurchschnitt — '
        ...
        if isinstance(_roadmap_90d, str):
            _longterm = [
                (r'12\s*Monate?', '3 Monate'),
                (r'(?i)ein\s+Jahr', '90 Tage'),
                (r'(?i)langfristig', 'kurzfristig'),
                (r'12\s*months?', '3 months'),
                (r'(?i)one\s+year', '90 days'),
            ]
            _c4_changed = False
            for _pattern, _replacement in _longterm:
                _new, _n = _re_b35.subn(_pattern, _replacement, _roadmap_90d)
                if _n > 0:
                    _roadmap_90d = _new
                    _c4_changed = True
            if _c4_changed:
