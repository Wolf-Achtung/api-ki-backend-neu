...
<p><strong>Benchmark:</strong> Durchschnitt {benchmark_avg}/100 · Top-Quartil {benchmark_top}/100</p>
    ...
    if recommendations_html and len(recommendations_html) > 200:
        try:
            original_length = len(recommendations_html)
            recommendations_html = _format_recommendations_compact(recommendations_html)
            log.info(f"[INTEGRATION] Recommendations HTML after compact cards: {len(recommendations_html)} chars (delta: {len(recommendations_html) - original_length})")
            sections["RECOMMENDATIONS_HTML"] = recommendations_html
        except Exception as e:
            log.error(f"[INTEGRATION] Recommendations compact formatting failed: {e}")
            # Keep original - don't break pipeline
