...
class TestDritteProgrammliste:
    """KIS-1272: services/funding_engine_v2.py fuehrt eine dritte,
    fest im Code stehende Programmliste (24 Eintraege). Sie wird von
    keinem Report genutzt — importiert wird das Modul nur von seiner
    eigenen Testdatei.

    Solange es liegen bleibt, ist die Gefahr nicht, dass es falsch
    rechnet, sondern dass seine Daten still veralten und jemand es
    spaeter verdrahtet. Diese Tests halten den Zustand fest.
    """

    def _engine_zim(self):
        from services.funding_engine_v2 import FUNDING_PROGRAMMES_2025_2027
        return next(p for p in FUNDING_PROGRAMMES_2025_2027
                    if "ZIM" in p.get("name", ""))

    def test_modul_wird_von_keinem_report_genutzt(self):
        """Aendert sich das, gehoert die Statusregel dort ebenfalls
        angewandt — dieser Test macht den Wechsel sichtbar."""
        from pathlib import Path
        repo = Path(__file__).resolve().parent.parent
        nutzer = []
        for ordner in ("services", "routes", "workers"):
            for pfad in (repo / ordner).rglob("*.py"):
                if pfad.name == "funding_engine_v2.py":
                    continue
                text = pfad.read_text(encoding="utf-8")
                for zeile in text.splitlines():
                    if "funding_engine_v2" in zeile and not zeile.strip().startswith("#"):
                        nutzer.append(f"{pfad.name}: {zeile.strip()[:60]}")
        assert not nutzer, (
            "funding_engine_v2 wird jetzt genutzt — Statusregel dort pruefen: "
            f"{nutzer}"
        )

    def test_zim_ist_auch_dort_als_gestoppt_vermerkt(self):
        """Beide Listen muessen sich einig sein, dass ZIM nicht
        beantragbar ist — sonst driften sie auseinander."""
        zim = self._engine_zim()
        assert zim["deadline_urgency"] == "paused"
        assert "07.07.2026" in zim["deadline"]

    def test_zim_faellt_dort_nicht_als_top_empfehlung_an(self):
        """Die fit-Werte wurden bewusst gesenkt (KIS-1286)."""
        zim = self._engine_zim()
        for feld in ("fit_solo", "fit_team", "fit_kmu"):
            assert zim[feld] <= 0.5, f"{feld}={zim[feld]} zu hoch fuer ein gestopptes Programm"
