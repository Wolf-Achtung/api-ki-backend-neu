...
@router.get("/{report_id}", response_model=ReportOut, dependencies=[Depends(rate_limiter("report:get", 30, 60))])
def get_report(report_id: int, db: Session = Depends(get_db)) -> ReportOut:
    rep = db.get(Report, report_id)
    if not rep:
        raise HTTPException(status_code=404, detail="Report not found")
    return ReportOut(id=rep.id, status=rep.status, pdf_url=getattr(rep, "pdf_url", None), analysis_id=getattr(rep, "analysis_id", None))
