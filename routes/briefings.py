...
- Legt Briefing ab, stößt Analyse im Background an
"""
import json
...
async def submit_briefing(request: Request, background: BackgroundTasks) -> JSONResponse:
    # CI dry‑run?
    if request.headers.get("x-dry-run", "").strip() in {"1","true","yes"}:
    ...
    if "application/json" in content_type:
        ...
        # answers kann als JSON‑String kommen
    ...
    try:
        ...
        background.add_task(run_async, briefing_id=b.id, email=answers.get("email") or answers.get("kontakt_email"))
