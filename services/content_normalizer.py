...
try:
    from .sanitize import ensure_utf8  # optional
except Exception:
...
def _parse_budget_range(text: str) -> float:
    ...
    if m:
        low, high = float(m.group(1)), float(m.group(2) or m.group(1))
        return (low + high) / 2.0
    try:
        return float(text)
    except Exception:
...
def _roi_and_costs(briefing: Dict[str, Any], metrics: Dict[str, Any]) -> Dict[str, str]:
    ...
    rate = float(metrics.get("stundensatz_eur", 60))
...
def _so_what(scores: Dict[str,int]) -> str:
    g, s, v, e = scores.get('governance',0), scores.get('security',0), scores.get('value',0), scores.get('enablement',0)
    items = []
    if g < 50: items.append("<li><strong>Governance:</strong> Rollen & Leitlinien klären (1–2 Seiten), Freigabepfade definieren.</li>")
    if s < 50: items.append("<li><strong>Sicherheit:</strong> Datenschutz-Checkliste + Logging, Prompt‑Richtlinien, Human‑Oversight.</li>")
    if v < 50: items.append("<li><strong>Nutzen:</strong> 3 Quick Wins priorisieren, KPI‑Baseline setzen, 30‑Tage‑Review.</li>")
    if e < 50: items.append("<li><strong>Befähigung:</strong> Prompt‑Training (3 Sessions), Brown‑Bags, Champions benennen.</li>")
    if not items:
...
def _default_tools_and_funding(briefing: Dict[str, Any], last_updated: str, report_date: str) -> Dict[str, str]:
    ...
    funding_rows = []
    if b == "be":
        funding_rows.extend([("KOMPASS (Solo‑Selbständige)","ESF+/BA","bis 29.02.2028","Qualifizierung/Coaching bis 4.500 €"),
        ...
        funding_rows.extend([("INQA‑Coaching (KMU)","BMAS/ESF+","laufend","80 % Zuschuss bis 12 Tage"),
...
def normalize_and_enrich_sections(briefing: Dict[str, Any] = None,
    ...
    if not out.get("ROI_HTML") or len(out.get("ROI_HTML","").strip()) < 20:
        roi_costs = _roi_and_costs(briefing, metrics)
        out.update(roi_costs)

    # Sensitivitätstabelle (E)
    invest = float(out.get("invest_value", 0) or 0)
    monthly = float(metrics.get("monatsersparnis_eur", 0) or 0)
    rate = float(metrics.get("stundensatz_eur", 60))
    if not out.get("BUSINESS_SENSITIVITY_HTML"):
        out["BUSINESS_SENSITIVITY_HTML"] = _sensitivity_table(invest, monthly, rate)

    # Tools/Förderungen + Stand
    ...
    if not out.get("TOOLS_HTML") or len(out.get("TOOLS_HTML","").strip()) < 24:
        out.update(_default_tools_and_funding(briefing, last_updated, briefing.get("report_date","")))
